...
begin
  ...
  begin
     ...
     try
       ...
       begin
         sql.Add('INSERT INTO produtos (cod_produto,descricao,preco_venda) '
                +' values ('''+inttostr(vproduto.cod_produto) +''','''+vproduto.descricao
...
function Tprodutos.deletarProduto(idproduto : integer) : Boolean;         // método excluir produto
var FB_Query : TFDQuery;
    sucesso :  boolean;
begin
  FB_Query := TFDQuery.Create(nil);
...
begin
  ...
  begin
     try
       ...
       close;
       sql.clear;
       if (tipo='lista') then                                                            // sql de busca
          ...
          sql.add('SELECT * from produtos where id = '+busca);

       open;
       while not eof do
         begin
           templistaproduto.FListaProdutos.Add(Tprodutos.Create);                                       // preenchelista de objetos de produtos com resultado sql
