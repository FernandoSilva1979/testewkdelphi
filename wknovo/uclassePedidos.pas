...
begin
  ...
  begin
     ...
     connection.StartTransaction;                    // controle de transação
     try
       close;
       sql.clear;
       if (acao='incluir') then
       begin
         sql.Add('INSERT INTO pedidos (data_emissao,cod_cliente,valor_total) '                   // sql
        ...
        begin
         sql.Add('update pedidos set data_emissao='''+formatdatetime('YYYY-MM-DD',vpedido.emissao)              // sql
...
function Tpedidos.deletarPedido(idpedido : integer) : Boolean;                // método de exclusão de pedido
var FB_Query : TFDQuery;
    sucesso :  boolean;
begin
  FB_Query := TFDQuery.Create(nil);
...
begin
  ...
  begin
     ...
     try
        ...
        sql.clear;
        if (tipo='lista') then
          sql.add('SELECT a.*,(select nome from clientes b where b.cod_cliente=a.cod_cliente) desccliente from pedidos a'       // sql
        ...
        while not eof do
         begin
             templistapedido.FListaPedidos.Add(Tpedidos.Create);                             // preencher lista de objeto com resultado do sql
    ...
    with FB_Query do
     begin
        ...
        try
         close;
         sql.clear;
         sql.Add('INSERT INTO pedidos_produtos (nro_pedido,cod_produto,quantidade,vlr_unitario,vlr_total) '           //sql
